...
export class UpgradeManager {
    ...
    private createUI() {
        ...
        this.createUpgradeBtn(upgradesDiv, "💥 Damage", () => this.damageLevel, () => {
            this.damageLevel++;
            if (this.onDamageUpgrade) this.onDamageUpgrade(this.damageLevel);
        });

        this.createUpgradeBtn(upgradesDiv, "⛏️ Mining Speed", () => this.mineSpeedLevel, () => {
            this.mineSpeedLevel++;
            if (this.onMineSpeedUpgrade) this.onMineSpeedUpgrade(1 + (this.mineSpeedLevel - 1) * 0.2);
        });

        this.createUpgradeBtn(upgradesDiv, "🏃 Move Speed", () => this.moveSpeedLevel, () => {
            this.moveSpeedLevel++;
