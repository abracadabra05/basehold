...
export class Game {
  ...
  public init() {
    ...
    this.upgradeManager.onMineSpeedUpgrade = (mul) => {
      this.currentMineMultiplier = mul;
      this.soundManager.playBuild();
    };
    this.upgradeManager.onMoveSpeedUpgrade = (mul) => {
      this.player.speedMultiplier = mul;
      this.soundManager.playBuild();
    };
  ...
  public spawnEnemy(x: number, y: number, type: EnemyType) {
    const target =
      this.coreBuilding && !this.coreBuilding.isDestroyed
        ? this.coreBuilding
        : this.player;
  ...
  private updateProjectiles(ticker: any) {
    for (const p of this.projectiles) {
      ...
      for (const enemy of this.enemies) {
        ...
        // ИСПОЛЬЗУЕМ ХИТБОКС ВРАГА + РАДИУС ПУЛИ (4)
        if (dist < enemy.hitboxRadius + 4) {
          enemy.takeDamage(p.damage);
          p.shouldDestroy = true;
          // (Звук можно добавить тут)
          break;
        }
      }
    }
